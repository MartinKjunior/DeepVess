...
if isForward:
    #Added the DeepVess folder as I'm running the code from one level up on the CSF.
    #Ideally this would be rewritten to programatically find the location of the file (even just using __file__)
    # and then finding the saved model from that location instead of counting on the cwd.
    saver.restore(sess, os.path.join('DeepVess','private', 'model-epoch29999.ckpt'))
